...
class PosOrderLine(models.Model):
    ...
    def write(self, vals):
        ...
        if 'x_studio_test' in vals:
            self._update_tea_order_line()
        return res
    ...
    def _update_tea_order_line(self):
        ...
        if (
            self.company_id.name in allowed_companies
        ):
...
class TeaOrderLine(models.Model):
    ...
    state_1 = fields.Char(string="state_1", compute="_get_stat")
    ...
    ReceiptNo_2 = fields.Char(string="Receipt No")
    SalesCurrency = fields.Char(string="Sales Currency")
    TotalSalesAmtB4Tax = fields.Float(
        string="Line Total Excl. tax",
        compute="_compute_b4_tax_rate",
        store=True,
    )
    ...
    @api.depends('ReceiptNo')
    def _get_stat(self):
        for record in self:
            # Search for matching pos.order.line records
            pos_order_lines = self.env['pos.order.line'].search([('name', '=', record.ReceiptNo_2)])
            
            if pos_order_lines:
                # Assuming you want to set state_1 from the first matching record
                record.state_1 = pos_order_lines[0].x_studio_test
                if record.state_1 in ['draft', 'cancelled']:
                    record.status = 'Manual'
            else:
                record.state_1 = 'Not Found'
    ...
    def button_check(self):
        ...
        client_id = date_record.customer_id
        client_secret = date_record.secret_client
        access_token_url = date_record.token_url
        data_url = date_record.ogf_url
        app_code = date_record.app_code
        property_code = date_record.property_code
        ...
        if records:
            ...
            for record in records:
                if record.status == "Open" and record.company in [
                    "Tea Tang - One Galle Face Mall",
                    "SHA Foundation - One Galle Face Mall",
                ]:
