...
class TeaOrderLine(models.Model):
    ...
    TotalSalesAmtB4Tax = fields.Float(string="Total Sales Before Tax")
    TotalSalesAmtAfterTax = fields.Float(string="Total Sales After Tax")
    ...
    ItemAmt = fields.Float(string="Item Amount")
    ...
    AmtTax = fields.Float("Total Tax")
    TaxRate = fields.Char(string="Tax Rate", default="18")
    ...
    #         for item in items:
    #             batch_numbers[item.id] = batch_code
    #         # Convert JSON data to a string
    #         json_data_str = json.dumps(json_data, indent=4)
    ...
    #                         if record.id in batch_numbers:
    #                             record.write(
    #                                 {
    #                                     "status": "Success",
    #                                     "batch": batch_numbers[record.id],
    #                                 }
    #                             )
    #                         else:
    #                             # Handle the case where the record doesn't have a batch number yet
    #                             pass
    ...
    def button_check(self):
        ...
        if records:
            ...
            for session_id, items in session_records.items():
                ...
                # Compare the date
                if items[0].ReceiptDate < comparison_date:
                    continue
                # receipt_no = items[0].ReceiptNo
                session_items = {
                    "PropertyCode": "CCB1",
                ...
                # Just sending the payment method as per the store
                payment_methods = items[0].PaymentMethods.split(
                    ", "
                )  # Split the payment methods
                renamed_payment_methods = [
